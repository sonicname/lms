...
export default function VideoPlayer({
  ...
  const [volume, setVolume] = useState<number>(() => {
    const saved = localStorage.getItem('sg_video_volume');
  ...
  useEffect(() => {
    localStorage.setItem('sg_video_volume', String(volume));
  }, [volume]);
  ...
  const onKeyDown: React.KeyboardEventHandler<HTMLDivElement> = (e) => {
    ...
    // schedule auto-hide after keyboard interaction
  ...
  useEffect(() => {
    // when play starts, schedule hide; when paused, always show controls
    if (playing) {
      scheduleAutoHide();
    } else {
      clearHideTimer();
