...
export function buildGlobRouteConfig(MODULES: GlobModules): RouteConfigNode[] {
  ...
  const buildNodes = (node: DirNode): RouteConfigNode[] => {
    ...
    for (const child of node.children.values()) {
      ...
      if (child.layoutFile) {
        ...
        // No layout: create a virtual segment with children. Use a physical file placeholder.
        out.push({
          path: childPath,
          file: 'modules/__virtual.tsx',
          children: childrenConfig,
        });
      }
    }
